...
def plot(tar_path,emptycav_plot=False):
    ...
    sa_T=np.zeros((sizef,3))#Avg transmission of SA vs freqs
    sa_R=np.zeros((sizef,3))
    nsa_T=np.zeros((sizef,3))#Avg transmission of non-SA vs freqs
    ...
    for i,f in enumerate(Tfiles):
        ...
        Ta=np.genfromtxt(T_path)[1:,:]#Timetrace of transmission for one atom
...
#######################################
    ...
    #REPORT WRITING
    mt.mkdir(tar_path+'/report')
    with open(tar_path+'/report/report_spectrum','w') as f:
        f.write('{:^10} {:^10} {:^10} {:^10} {:^10} {:^10} {:^10} {:^10} \n'.format('freq','avg_T','std_T','stderr_T','avg_R','std_R','stderr_R','Nofatom'))
        for i in range(len(freq)):
            f.write('{:^10} {:^10.3f} {:^10.3f} {:^10.3f} {:^10.3f} {:^10.3f} {:^10.3f} {:^10d} \n'.format(int(freq[i]),sa_T[i,0],sa_T[i,1],sa_T[i,1]/np.sqrt(sa_T[i,2]),sa_R[i,0],sa_R[i,1],sa_R[i,1]/np.sqrt(sa_R[i,2]),int(nof_sa_atom[i])))
    
    with open(tar_path+'/report/report','w') as f:
        f.write('Number of data sets: {:^3}\n'.format(mt.count_subfolder(tar_path+'/data')))
        f.write('Number of freqs: {:^3}\n'.format(np.size(f) ))
        f.write('SA filter method used:\n')
